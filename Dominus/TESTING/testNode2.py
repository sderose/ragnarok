...
class TestDOMNode(unittest.TestCase):
    ...
    def test_node_removal(self):
        # Make sure child2 is hooked up right to start with
        assert self.n.child2 is not None
        assert self.n.child2.parentNode == self.n.docEl
        assert self.n.child2 in self.n.docEl.childNodes
        nch = len(self.n.docEl.childNodes)
        nextSib = self.n.child2.nextSibling

        if (debug): DBG.dumpChildNodes(self.n.docEl, "removing child2", True)
        removedNode = self.n.docEl.removeChild(self.n.child2)
        self.assertEqual(removedNode, self.n.child2)
        if (debug): DBG.dumpChildNodes(self.n.docEl, "after", True)
        if (self.n.child2 in self.n.docEl.childNodes):
            sys.stderr.write("    WHAT? pn None? %s (child2 id %x)\n"
                % (self.n.child2.parentNode is None, id(self.n.child2)))
            DBG.dumpChildNodes(self.n.docEl, "after", True)

        self.assertEqual(len(self.n.docEl.childNodes), nch - 1)
        self.assertIsNone(self.n.child2.parentNode)
        self.assertEqual(self.n.child1.nextSibling, nextSib)
        if (nextSib is not None):
            self.assertEqual(nextSib.previousSibling, self.n.child1)
        self.assertFalse(self.n.child2 in self.n.docEl.childNodes)

        # Remove text node
        self.n.child1.removeChild(self.n.textNode1)
        self.assertFalse(self.n.child1.hasChildNodes())
    ...
    def test_previous_sibling(self):
        self.assertEqual(self.n.child2.previousSibling, self.n.child1)
        self.assertIsNone(self.n.child1.previousSibling)

    def test_next_sibling(self):
        self.assertEqual(self.n.child1.nextSibling, self.n.child2)
        self.assertIsNone(self.n.docEl.childNodes[-1].nextSibling)

    def test_attributes(self):
        self.assertIsNotNone(self.n.child1.attributes)
        self.assertEqual(self.n.child1.attributes[DAT.at_name].value, DAT.at_value)
    ...
    def test_has_attributes(self):
        self.assertTrue(self.n.child1.hasAttributes())
        self.assertFalse(self.n.child2.hasAttributes())
    ...
    def test_normalize(self):
        ...
        self.n.child1.appendChild(newTextNode)
        self.assertEqual(len(self.n.child1.childNodes), 2)
        lg.info("2 text nodes? %s",
            [ ("%s: '%s'" % (x.nodeName, x.data)) for x in self.n.child1.childNodes ])

        self.n.child1.normalize()
        lg.info("   post norm  %s",
            [ ("%s: '%s'" % (x.nodeName, x.data)) for x in self.n.child1.childNodes ])
        self.assertEqual(len(self.n.child1.childNodes), 1)
        self.assertEqual(self.n.child1.firstChild.nodeValue,
    ...
    def test_is_same_node(self):
        ...
        self.assertFalse(self.n.docEl.isSameNode(self.n.child1))
